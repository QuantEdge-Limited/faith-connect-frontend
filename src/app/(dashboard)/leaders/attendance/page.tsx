...
type AttendanceStatus = 'present' | 'absent-with-apology' | 'absent-without-apology';
...
export default function AttendanceManagementPage() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const storedData = localStorage.getItem(`parish_attendance_${selectedDate}`);
      if (storedData) {
        try {
          const parsed = JSON.parse(storedData);
          // Validate data structure
          const isValid = PARISH_MEMBERS.every(member => 
            parsed[member.id] && 
          ...
          console.warn('Invalid attendance data in localStorage, resetting...');
        }
      }
    }
  ...
  const getFilteredMembers = (): ParishMember[] => {
    ...
    return PARISH_MEMBERS.filter(member => attendanceRecords[member.id] === filterStatus);
  };
  ...
  const getMemberStatusCount = (status: AttendanceStatus): number => {
    return PARISH_MEMBERS.filter(member => attendanceRecords[member.id] === status).length;
  };
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            ...
            {(Object.keys(ATTENDANCE_STATUS_CONFIG) as AttendanceStatus[]).map(status => (
              ...
              >
                {ATTENDANCE_STATUS_CONFIG[status].icon} {ATTENDANCE_STATUS_CONFIG[status].label}{' '}
                    ...
                    <td className="py-4 px-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          ATTENDANCE_STATUS_CONFIG[attendanceRecords[member.id] || 'present'].color
                        }`}
                      >
                        {ATTENDANCE_STATUS_CONFIG[attendanceRecords[member.id] || 'present'].icon}
                        <span className="ml-1">
                          {ATTENDANCE_STATUS_CONFIG[attendanceRecords[member.id] || 'present'].label}
                        </span>
                      </span>
                    </td>
                    <td className="py-4 px-4 whitespace-nowrap">
                      ...
                      >
                        ...
                        <option value="absent-without-apology">⚠️ Absent (no apology)</option>
                      </select>
                    </td>
                  </tr>
                ))}
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            ...
            className={`inline-flex items-center px-6 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors ${
              !selectedDate || !isSunday(selectedDate) || !hasUnsavedChanges || saveStatus === 'saving'

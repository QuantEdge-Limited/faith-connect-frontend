  ...
  return (
    <Card className="bg-white border-2 border-[#D4AF37] rounded-xl p-6 hover:shadow-xl transition-all duration-300 flex flex-col">
      ...
      <div className="flex items-start">
        {Icon && (
          <div className="bg-[#D4AF37] bg-opacity-10 p-3 rounded-lg mr-4 flex-shrink-0">
            <Icon className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
          </div>
        )}
        <div className="flex-1 min-w-0">
          <h3 className="text-lg sm:text-xl font-bold text-[#0D090A] break-words mb-1">
            {title}
          </h3>
          {subtitle && (
      ...
      {/* Description */}
      {description && (
        <p className="text-[#0D090A] text-sm sm:text-base flex-grow">
          {description}
        </p>
      )}
      ...
      {metadata && metadata.length > 0 && (
        <div className="space-y-2 pt-4 border-t border-[#D4AF37] border-opacity-20">
          {metadata.map(({ icon: MetaIcon, label }, idx) => (
            ...
            >
              <MetaIcon className="w-4 h-4 mr-2 text-[#D4AF37] flex-shrink-0" />
